...
if let name = optionalName {
    greeting = "Hello, \(name)"     // if optionalName is nil this line of code will not run
}else{
    greeting = "Hi, Mr.nil"
}
...
switch vegetable {
    ...
    // I will get the error "switch must be exhaustive" if I remove the default case.
    // so I will leave the default case here
}
...
for (name, numbers) in interestingNumbers {
    for number in numbers {
        if number > largest {
            what_name = name        //change the name when find the large number
            largest = number
        }
    }
}
